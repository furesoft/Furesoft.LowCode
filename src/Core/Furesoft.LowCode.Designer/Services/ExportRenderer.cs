...
internal static class ExportRenderer
{
    ...
    {
        using var drawingContextImpl = DrawingContextHelper.WrapSkiaCanvas(canvas, new(dpi, dpi));
    ...
    public static void RenderSvg(Control target, Size size, Stream stream, double dpi = 96)
    {
        ...
        var bounds = SKRect.Create(new((float)size.Width, (float)size.Height));
