        ...
        if (DataContext is IConnector connector)
        {
            ...
            if (connector.Start.Parent == connector.End.Parent)
            {
                if ((connector.Start.Alignment == PinAlignment.Top && connector.End.Alignment == PinAlignment.Bottom)
                    || (connector.Start.Alignment == PinAlignment.Bottom &&
                        connector.End.Alignment == PinAlignment.Top))
                {
                ...
                else if (connector.Start.Alignment == PinAlignment.Right && connector.End.Alignment == PinAlignment.Top)
                {
                    context.LineTo(new(p1X, p1Y - connector.Start.Parent.Height));
                    context.LineTo(new(p2X, p1Y - connector.Start.Parent.Height));
                }
                else if (connector.Start.Alignment == PinAlignment.Top && connector.End.Alignment == PinAlignment.Right)
                {
                    context.LineTo(new(p1X + connector.Start.Parent.Width, p1Y - connector.Start.Parent.Height / 2));
                    context.LineTo(new(p2X, p2Y));
                }
            }
            else
