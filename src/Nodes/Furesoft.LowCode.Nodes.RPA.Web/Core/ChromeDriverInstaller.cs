...
public class ChromeDriverInstaller
{
    ...
    {
        BaseAddress = new("https://chromedriver.storage.googleapis.com/")
    };
    ...
    public async Task Install(string? chromeVersion, bool forceDownload)
    {
        ...
        if (!chromeDriverVersionResponse.IsSuccessStatusCode)
        {
            ...
            {
                throw new($"ChromeDriver version not found for Chrome version {chromeVersion}");
            }
            else
            {
                throw new(
        ...
        if (!forceDownload && File.Exists(targetPath))
        {
            ...
            {
                throw new($"Failed to execute {driverName} --version");
            }
        }
        ...
        if (RuntimeInformation.IsOSPlatform(OSPlatform.Linux) || RuntimeInformation.IsOSPlatform(OSPlatform.OSX))
        {
            ...
            if (!string.IsNullOrEmpty(error))
            {
                throw new("Failed to make chromedriver executable");
            }
        }
    }
    ...
    public async Task<string> GetChromeVersion()
    {
        if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
        {
            var chromePath = (string?)Registry.GetValue(
                "HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\App Paths\\chrome.exe", null, null);
            if (chromePath == null)
            {
                throw new("Google Chrome not found in registry");
            }
        ...
        else if (RuntimeInformation.IsOSPlatform(OSPlatform.Linux))
        {
            ...
            {
                throw new("An error occurred trying to execute 'google-chrome --product-version'", ex);
            }
        }
        else if (RuntimeInformation.IsOSPlatform(OSPlatform.OSX))
        {
