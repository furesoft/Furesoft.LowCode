...
public static class SeleniumExtensions
{
    ...
    public static HtmlDocument GetPageDocument(this ISearchContext element)
    {
        var doc = new HtmlDocument();
        var html = "";
        if (element is IWebElement e)
            html = e.GetAttribute("outerHtml");
        if (string.IsNullOrWhiteSpace(html))
            html = ((IWebElement)element).GetAttribute("outerHTML");
        if (element is IWebDriver d)
            html = d.PageSource;
    ...
    {
        var remote = (RemoteWebElement)element;
        return remote.WrappedDriver.ExecuteJavaScript<IWebElement>($"return $(arguments[0]).find(\"{cssSelector}\")[0]",
            element);
    }
    ...
    public static async Task<IQueryable<HtmlNode>?> GetHtmlNodes(this ISearchContext element,
    ...
    public static async Task<HtmlNode> GetHtmlNode(this ISearchContext element, Func<HtmlNode, bool> query,
        int index = 0, TimeSpan timeout = default)
    {
        if (timeout == default)
            timeout = TimeSpan.FromSeconds(20);
        if (index < 0)
    ...
    public static async Task<IWebElement> FindElement<TChild>(this IWebElement element, Func<HtmlNode, bool> query,
        int index = 0, TimeSpan timeout = default)
    {
        var node = await GetHtmlNode(element, query, index, timeout);
        if (node == null)
            throw new Exception("Element not found. Check searching criterias");
    ...
    /// </summary>
    public static async Task<IWebElement> FindElement(this ISearchContext element, Func<HtmlNode, bool> query,
        int index = 0, TimeSpan timeout = default)
    {
        var node = await GetHtmlNode(element, query, index, timeout);
    ...
    public static async Task<IEnumerable<IWebElement>> FindElements(this IWebDriver element, Func<HtmlNode, bool> query,
        TimeSpan timeout = default)
    {
    ...
    public static async Task<IEnumerable<IWebElement>> FindElements<TChild>(this IWebElement element,
        Func<HtmlNode, bool> query, TimeSpan timeout = default)
    {
        var output = new List<IWebElement>();
        var nodes = await GetHtmlNodes(element, timeout);
        if (nodes == null)
